"""
Cleanup script for challenge master test
Removes the challenge_master@gomums.com user and all related data
"""
import os
import psycopg2
...
TEST_EMAIL = "challenge_master@gomums.com"
...
def cleanup_test_user():
    ...
    try:
        ...
        tables_to_clean = [
            ('user_challenge_goals', 'user_challenge_id', 'SELECT id FROM user_challenges WHERE user_id = %s'),
        ...
        for table_info in tables_to_clean:
            if len(table_info) == 3:
                # Special case: need to get IDs from another table first
                table, column, subquery = table_info
                cursor.execute(f"DELETE FROM {table} WHERE {column} IN ({subquery})", (user_id,))
            else:
        ...
        print(f"\n✅ Cleanup complete! You can now run test_all_challenges.py")
